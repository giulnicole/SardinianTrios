...
# Loading data 
load("checkpoint_knockoff/ordered_geno-haplo_before_knockoff.RData")
...
# Reading data 
ordered_geno <- data_knockoff[[4]]
dim(ordered_geno)
order.id <- rownames(ordered_geno)

ordered_haplo <- data_knockoff[[3]]

ordered_haplo2 <- as.matrix(ordered_haplo)


# Analaysis on trios 

# 1. Generating nockoff haplotypes(under null hypothesis)
library(KnockoffTrio)
pos <- data_knockoff[[1]]

res_knock8 <- create_knockoff(ordered_haplo2, pos, maxcor=0.8, M=8)

ordered_trios <- data_knockoff[[2]]
